...
// #hooks :
import { getPostsByFeatured } from "actions/FetchPosts";
...
export async function getServerSideProps(context) {
  const posts = await getPostsByFeatured({
    context: context,
  });
  return {
    props: {
          ...
          <Box mx={width === "xs" ? 1 : 7} maxWidth={1700} width="100%">
            <Card style={{ padding: "32px 0px" }}>
              <Box
                ...
                flexShrink={1}
              >
                <Typography className={classes.page_title}>Featured</Typography>
                <Typography className={classes.page_subtitle}>
                  Posts in this page is featured and editor choice.
                </Typography>
              </Box>
            </Card>
          </Box>
